...
router.get('/viewfriends',loggedin, async function (req,res,next) {
    ...
    let result = await connection.query("SELECT friend_email, status FROM friends WHERE email = $1 ORDER BY status ASC", [req.session.email]);
...
router.post('/friendrequests', loggedin, function (req,res,next) {
    ...
    if(status == 2){ //Accept case
        connection.query("UPDATE friends SET status = 0 WHERE email = $1 AND friend_email = $2", [req.session.email, friendemail]);
        connection.query("UPDATE friends SET status = 0 WHERE email = $1 AND friend_email = $2", [friendemail, req.session.email]);
    }
    //Either cancel, or delete friend/friend req. Does same thing to database
    else{
        connection.query("DELETE FROM friends WHERE email = $1 AND friend_email = $2",[req.session.email, friendemail])
        connection.query("DELETE FROM friends WHERE email = $1 AND friend_email = $2",[friendemail, req.session.email])
    }
...
router.get('/searchFriends',loggedin, async function(req,res,next) {
    ...
    if (searchFriends) {
        let result = await connection.query("SELECT * FROM personal_details WHERE (first_name ILIKE $1 OR last_name ILIKE $1) AND email NOT IN(SELECT friend_email FROM friends WHERE email = $2) AND email <> $2 ", [searchFriends, req.session.email]);
